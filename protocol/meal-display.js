  ...
  function createProtocolMealDisplayAdapter(options) {
    ...
    function buildPlanHtml(meal) {
      ...
      const planItems = meal.plan.items.map(item => `
          <div class="flex items-baseline justify-between border-b border-ink/12 py-1 text-[0.62rem] last:border-b-0">
            ...
            <span>
              ...
              ${item.kcal > 0 ? `<span class="ml-2 text-[0.55rem] text-stone tabular-nums">${item.kcal} kcal</span>` : ''}
            </span>
          </div>`).join('');
      ...
      return `
          <div class="mt-2 mb-1">
            ${planItems}
            <div class="mt-2 flex justify-between border-t border-warm-light pt-2 text-[0.65rem]">
              <span class="text-[0.55rem] tracking-[0.06em] text-stone uppercase">Meal total</span>
      ...
      return `
        <div class="mt-2 mb-1 grid grid-cols-2 gap-[2px_12px]">
          ${meal.ingredients.map(item => `
            <div class="flex items-baseline gap-[5px] text-[0.6rem] leading-[1.6] tracking-[0.02em] text-stone">
              <span class="shrink-0 text-[0.58rem] whitespace-nowrap text-warm tabular-nums">${item.amount}</span>
              <span>${item.name}</span>
            </div>`).join('')}
        </div>
        <div class="my-2 h-px bg-ink/12"></div>`;
    ...
    function buildCardHtml(meal, isSelected, showPlan) {
      ...
      return `
        <button class="meal-copy-btn absolute top-3 right-[14px] z-2 flex size-[26px] cursor-pointer items-center justify-center rounded-[3px] border border-ink/12 bg-white" title="Copy ingredients">
        ...
        <div class="mb-1 flex items-baseline gap-2.5 pr-[34px]">
          <div class="flex-1 text-[0.78rem] tracking-[0.02em] text-ink">${meal.name}${meal.plan ? '<span class="ml-1.5 inline-block rounded-[2px] bg-signal px-1.5 py-[2px] align-middle font-mono text-[0.48rem] tracking-widest text-white uppercase not-italic">1400</span>' : ''}</div>
          <div class="shrink-0 text-[0.6rem] tracking-[0.06em] whitespace-nowrap text-warm tabular-nums">${meal.kcal} kcal</div>
        </div>
        <div class="mb-1 text-[0.58rem] tracking-[0.04em] text-stone">
          <span class="mr-2.5">P ${meal.protein}g</span><span class="mr-2.5">C ${meal.carbs}g</span><span>F ${meal.fat}g</span>
        </div>
        ${contentHtml}
        <div class="font-display text-[0.62rem] leading-normal text-stone italic">${meal.notes}</div>
        ${meal.tags.map(tag => `<span class="mt-[5px] mr-1 inline-block rounded-[2px] px-[5px] py-px font-mono text-[0.48rem] tracking-widest uppercase not-italic ${tag === 'quick' ? 'text-[#5a6b7a] border border-[#5a6b7a]' : 'text-moss border border-moss'}">${tag}</span>`).join('')}
      `;
    }
